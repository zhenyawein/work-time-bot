...
async def show_year_selection(update, date_type: str):
    ...
    for year in range(current_year - 1, current_year + 2):  # -1, текущий, +1
        row.append(InlineKeyboardButton(str(year), callback_data=f"year_{year}_{date_type}"))
        if len(row) == 3:
            keyboard.append(row)
            row = []
    if row:
    ...
    if isinstance(update, Update):
        await update.message.reply_text(
            "📅 Выберите год для начальной даты отчета:",
            reply_markup=reply_markup
        )
    else:
        # Это callback query
...
async def show_month_selection(query, year: int, date_type: str):
    ...
    for i, month in enumerate(months, 1):
        row.append(InlineKeyboardButton(month, callback_data=f"month_{year}_{i:02d}_{date_type}"))
        if len(row) == 3:
            keyboard.append(row)
            row = []
    if row:
    ...
    keyboard.extend([
        [InlineKeyboardButton("◀️ Назад к выбору года", callback_data="report_start")],
        [InlineKeyboardButton("❌ Отмена", callback_data="report_cancel")]
    ])
    
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text(f"📅 Выберите месяц для {year} года:", reply_markup=reply_markup)
...
async def show_day_selection(query, year: int, month: int, date_type: str):
    ...
    for day in range(1, last_day.day + 1):
        row.append(InlineKeyboardButton(str(day), callback_data=f"day_{year}_{month:02d}_{day:02d}_{date_type}"))
        if len(row) == 7:
            keyboard.append(row)
            row = []
    if row:
    ...
    keyboard.extend([
        [InlineKeyboardButton("◀️ Назад к выбору месяца", callback_data=f"year_{year}_{date_type}")],
        [InlineKeyboardButton("❌ Отмена", callback_data="report_cancel")]
    ])
    ...
    await query.edit_message_text(f"📅 Выберите день ({month_name} {year}):", reply_markup=reply_markup)
...
def generate_report(period_data: Dict, start_date: str, end_date: str) -> str:
    ...
    report_lines = [f"📊 *ОТЧЕТ за период:* {format_date(start_date)} - {format_date(end_date)}\n"]
    ...
    for work_day in period_data['work_days']:
        ...
        if start_time and end_time and start_time != end_time:
            actual_hours, work_hours_with_lunch = calculate_work_hours(start_time, end_time)
...
def main():
    ...
    try:
        ...
        application.add_handler(MessageHandler(filters.Regex("🟢 Начало рабочего дня"), start_work_day))
        application.add_handler(MessageHandler(filters.Regex("🔴 Конец рабочего дня"), end_work_day))
        application.add_handler(MessageHandler(filters.Regex("📊 Отчет"), report_start))
        application.add_handler(MessageHandler(filters.Regex("📝 Добавить действие"), add_action_start))
        ...
        application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, add_action_complete))
        
        # Обработчики callback для перезаписи времени
        application.add_handler(CallbackQueryHandler(handle_overwrite_callback, pattern="^overwrite_|^cancel_overwrite"))
        
        # Обработчик inline кнопок для отчетов
        application.add_handler(CallbackQueryHandler(handle_report_callback, pattern="^report_|^year_|^month_|^day_|^quick_"))
